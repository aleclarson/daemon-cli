...
export async function createCommand(options: CreateOptions) {
  ...
  const rotation = options.rotation || (await select('Log rotation frequency?', [
    { label: 'daily', value: 'daily' },
    { label: 'weekly', value: 'weekly' },
    { label: 'hourly', value: 'hourly' }
  ]));

  const keepString = options.keep !== undefined ? String(options.keep) : (await prompt('Keep how many logs?', '7'));
  ...
  const finalPlistPath = await generatePlist(name, { keepAlive: options.keepAlive });
