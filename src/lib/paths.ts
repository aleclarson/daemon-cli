...
export const STATE_FILE = untildify('~/.local/state/daemon-cli/logrotate.status');

export const getWrapperPath = (name: string) => path.join(WRAPPERS_DIR, `${name}.sh`);
export const getConfigPath = (name: string) => path.join(LOGROTATE_DIR, `${name}.conf`);
export const getPlistPath = (name: string) => path.join(PLIST_DIR, `homebrew.mxcl.${name}.plist`);
