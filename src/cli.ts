...
if (!logrotatePath) {
  log.error(
    'logrotate is not installed. Please install it via "brew install logrotate".'
  )
  process.exit(1)
}
...
const completion = subcommands({
  ...
  cmds: {
    ...
    'list-daemons': command({
      name: 'list-daemons',
      description: 'Internal: List managed daemons for completion',
      args: {},
      handler: async () => {
        const names = await getManagedDaemonNames()
